...
public class Repository {
    ...
    public static void init() {
        if (GITLET_DIR.exists()) {
            System.out.println("A Gitlet version-control system already exists in the currant directory.");
            System.exit(0);
        }
        mkdir(GITLET_DIR);
        mkdir(OBJECT_DIR);
    ...
    private static void storeBlob(Blob blob) {
        ...
        if (!currCommit.getPathToBlobID().containsValue(blob.getBlobID()) || !removeStage.isNewBlob(blob) ) {
    ...
    public static void checkout(String fileName) {
        Commit currCommmit = readCurrCommit();
